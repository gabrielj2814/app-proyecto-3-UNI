...
class PermisoTrabajadorModelo extends DriverPostgre{
    ...
    async consultarPermisoTrabajadorFechaDesdeHastaModelo(){
        const SQL=`SELECT * FROM tpermisotrabajador,tpermiso,ttrabajador WHERE tpermisotrabajador.id_cedula=${this.id_cedula} fecha_desde_permiso_trabajador BETWEEN '${this.fecha_desde_permiso_trabajador}' AND '${this.fecha_hasta_permiso_trabajador} AND tpermisotrabajador.id_cedula=ttrabajador.id_cedula AND tpermisotrabajador.id_permiso=tpermiso.id_permiso;`
        console.log(SQL)
        return await this.query(SQL)
    }
}
